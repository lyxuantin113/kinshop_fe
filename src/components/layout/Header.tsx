...
import { ShoppingCart, User, Search, Menu, LogOut, ShieldCheck } from 'lucide-react';
    ...
    <header className="sticky top-0 z-50 w-full border-b border-slate-200 bg-white/80 backdrop-blur-md">
      <div className="container-custom flex h-16 items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          <button className="p-2 text-slate-600 hover:text-primary-600 lg:hidden">
            <Search className="h-5 w-5" />
          </button>
          
          <Link href="/cart" className="relative p-2 text-slate-600 hover:text-primary-600 transition-colors">
            <ShoppingCart className="h-5 w-5" />
            <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-accent-500 text-[10px] font-bold text-white">0</span>
          </Link>
          ...
          {user ? (
            <div className="flex items-center space-x-4">
              ...
              <Link href="/orders" className="hidden sm:flex items-center space-x-1 p-2 text-slate-600 hover:text-primary-600 transition-colors">
                ...
                <span className="text-sm font-medium truncate max-w-[100px]">{user.fullName}</span>
              </Link>
              <button 
                onClick={logout}
          ...
          <button className="p-2 text-slate-600 hover:text-primary-600 md:hidden">
